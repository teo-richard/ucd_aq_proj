...
mean(df_full_clean_catedu$wtp_paqi)
mean(df_full_clean_catedu$wtp_epd)
mean(df_full_clean_catedu$wtp_dif)
sd_wtp_df = sd(df_full_clean_catedu$wtp_dif) # 24.65408
24.65408 / 228
...
-32.039 / 24.65408
# Number of SDs: -1.299541

# Interpretation: This treatment effect of about -32 PKR represents 1.3 standard deviations of the relative WTP distribution (SD = 24.65408). 
...
ggplot(df_full_clean_catedu) +
    ...
    geom_vline(xintercept = mean(df_full_clean_catedu$wtp_dif) - coef, color = "black") +
    geom_vline(xintercept = mean(df_full_clean_catedu$wtp_dif) + coef, color = "black") +
    geom_vline(xintercept = mean(df_full_clean_catedu$wtp_dif) - coef - sd_wtp_df, linetype = "dashed", color = "#ff8737") +
    geom_vline(xintercept = mean(df_full_clean_catedu$wtp_dif) - coef + sd_wtp_df, linetype = "dashed", color = "#ff8737") +
    geom_vline(xintercept = mean(df_full_clean_catedu$wtp_dif) + coef - sd_wtp_df, linetype = "dashed", color = "#8564e8") +
    geom_vline(xintercept = mean(df_full_clean_catedu$wtp_dif) + coef + sd_wtp_df, linetype = "dashed", color = "#8564e8") +
    labs(
        x = "Relative WTP",
        y = "Density",
...
ggsave("linear_regression_treatment_effect/treatment_effect_distr.png", width = 8, height = 4, dpi = 300)
